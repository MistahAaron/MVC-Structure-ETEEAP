...
function getApplicantId() {
  ...
  if (!id) {
    ...
    setTimeout(() => {
      window.location.href = '/frontend/AdminSide/3.adminApp/adminApp.html';
    }, 2000);
    return null;
  }
...
function updateStatusBadge(status) {
  ...
  if (status === 'Approved') {
    ...
    badge.classList.add('status-Disapproved');
  } else {
...
async function showAssignAssessorModal() {
  ...
  document.getElementById('disapproveBtn')?.addEventListener('click', async () => {
    ...
    showLoading();
    try {
      const response = await fetch(`${API_BASE_URL}/api/admin/applicants/${applicantId}/disapprove`, {
        method: 'POST',
        credentials: 'include'
      });
      ...
      if (data.success) {
        showNotification('Application Disapproved successfully!', 'success');
        updateStatusBadge('Disapproved');
        if (currentApplicant) currentApplicant.status = 'Disapproved';
      } else {
        throw new Error(data.error || 'Failed to disapprove application');
      }
    } catch (error) {
      console.error('Error disapproveing application:', error);
      showNotification(error.message, 'error');
    } finally {
      hideLoading();
    }
